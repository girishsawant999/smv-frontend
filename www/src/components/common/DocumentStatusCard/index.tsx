...
const DocumentStatusCard = ({name, status, progress, relation, numOfCompletedDocs, totalNumOfDocs}: IDocumentStatusCardProps) => {
    
    const getStatusText = ():string => {
        switch(status) {
            case "notStarted": return "Yet to start";
            default: return "";
        }
    }
    return (
        <div className={styles.docStatusCard + " px-5 py-5 my-5 cursor-pointer"}>
            <div className="flex mb-5">
                ...
                <div className="w-4/12 text-right relative">
                    <Typography weight="semi-bold" size="12" variant="p" className={"absolute bottom-0 right-0 " + styles.status}>
                        {getStatusText()}
                    </Typography>
                </div>
            </div>
